    ...
    <script type="text/javascript">
       let table;
        $(document).ready(function(){
            ...
            function __load_ticket_topic() {
                $.ajax({
                    url: 'include/tickets_server.php?get_ticket_topic_data=1', 
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
                        if (response.success==true) {
                            var topicOptions = '<label style="margin-left: 10px;"> ';
                            topicOptions += '<select class="topic_filter form-select select2">';
                            topicOptions += '<option>--Select--</option>';
                            
                        
                            $.each(response.data, function(key, data) {
                                topicOptions += '<option value="'+data.id+'">'+data.topic_name+'</option>';
                            });

                            topicOptions += '</select></label>';
                            
                            setTimeout(() => {
                                $('.dataTables_length').append(topicOptions);
                                $('.select2').select2();
                            }, 1000);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success==true) {
                            ...
                            /* Worker Dropdown */
                                var ticket_assign = `<label style="margin-left: 10px;">
                                    <select class="assign_id form-select" style="width: 150px; display: inline;">
                            ...
                            $('.dataTables_length').append(ticket_assign);
                            //$('.select2').select2();
                              //$('.assign_id').select2();
                            }, 500);
               ...
               "searching": true,
                ...
                "ajax"				: {
                    ...
                    data: function(d) {
                        ...
                        d.topic_id = $('.topic_filter').val() || '';
                       d.assign_id = $('.assign_id').val();
        ...
        /* Assign filter change event*/
        $(document).on('change', '.assign_id', function(){
            table.ajax.reload(null, false);
        }); 
        ...
        $("#add_complain_button_").on('click',function(e){
            e.preventDefault();
             var submitBtn = $("#ticket_complain_type_modal_form").find('button[type="submit"]');
            var originalBtnText = submitBtn.html();
            ...
            var form = $("#ticket_complain_type_modal_form");
            var url ="include/tickets_server.php?add_ticket_topic_data=true";
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        $('#addComplainTypeModal').modal('hide');
                        toastr.success(response.message);

                        ticket_complain_type();
                         submitBtn.html(`Add New Complain`);
                         submitBtn.prop('disabled', true);
                    } else {
